...
class _LifeCyclePageState extends State<LifeCyclePage> with SingleTickerProviderStateMixin {
  ...
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(humanLifeCycleQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(humanLifeCycleQuestions.length, 0);
      for (int i = 0; i < progress['humanLifeCycleCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('humanLifeCycleCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(humanLifeCycleQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < humanLifeCycleQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('humanLifeCycleCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('humanLifeCycleReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: humanLifeCycleQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(humanLifeCycleQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${humanLifeCycleQuestions.length}問',
