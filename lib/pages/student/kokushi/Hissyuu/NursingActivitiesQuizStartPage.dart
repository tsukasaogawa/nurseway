...
class NursingActivitiesQuizStartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalQuestions = nursingActivityPlacesQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('主な看護活動の場', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => NursingActivitiesPage(
