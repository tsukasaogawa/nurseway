...
class PatientFamilyQuizStartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalQuestions = nursingTargetQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('看護の対象としての患者と家族', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => PatientFamilyPage(
