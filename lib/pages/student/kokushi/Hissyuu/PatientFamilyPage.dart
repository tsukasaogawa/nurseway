...
class PatientFamilyPage extends StatefulWidget {
  ...
  @override
  _PatientFamilyPageState createState() => _PatientFamilyPageState();
}

class _PatientFamilyPageState extends State<PatientFamilyPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(nursingTargetQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(nursingTargetQuestions.length, 0);
      for (int i = 0; i < progress['nursingTargetCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('nursingTargetCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(nursingTargetQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < nursingTargetQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('nursingTargetCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('nursingTargetReviewList', questionIndex);
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('看護の対象としての患者と家族'),
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: nursingTargetQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(nursingTargetQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${nursingTargetQuestions.length}問',
