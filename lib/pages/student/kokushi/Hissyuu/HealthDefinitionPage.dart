...
class HealthDefinitionPage extends StatefulWidget {
  ...
  @override
  _HealthDefinitionPageState createState() => _HealthDefinitionPageState();
}

class _HealthDefinitionPageState extends State<HealthDefinitionPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(healthDefinitionQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(healthDefinitionQuestions.length, 0);
      for (int i = 0; i < progress['healthDefinitionCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('healthDefinitionCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(healthDefinitionQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < healthDefinitionQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('healthDefinitionCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('healthDefinitionReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: healthDefinitionQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(healthDefinitionQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${healthDefinitionQuestions.length}問',
