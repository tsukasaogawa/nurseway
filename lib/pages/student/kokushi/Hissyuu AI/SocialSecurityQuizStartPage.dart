...
class SocialSecurityQuizStartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalQuestions = nursingSocialSecurityQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('看護で活用する社会保障', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => SocialSecurityPage(
