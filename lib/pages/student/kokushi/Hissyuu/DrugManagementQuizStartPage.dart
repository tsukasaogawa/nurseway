...
class DrugManagementQuizStartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalQuestions = drugEffectsAndManagementQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('薬物の作用とその管理', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => DrugManagementPage(
