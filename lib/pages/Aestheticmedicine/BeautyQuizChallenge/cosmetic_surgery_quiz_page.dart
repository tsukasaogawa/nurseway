...
class CosmeticSurgeryQuizPage extends StatefulWidget {
  final int initialPage;
  final void Function(int completedQuestions) onProgressUpdate;

  const CosmeticSurgeryQuizPage({
    super.key,
  ...
  @override
  _CosmeticSurgeryQuizPageState createState() => _CosmeticSurgeryQuizPageState();
}

class _CosmeticSurgeryQuizPageState extends State<CosmeticSurgeryQuizPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(cosmeticSurgeryQuizQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(cosmeticSurgeryQuizQuestions.length, 0);
      for (int i = 0; i < progress['cosmeticSurgeryCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final ProgressService progressService = ProgressService();
    await progressService.saveProgress('cosmeticSurgeryCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(cosmeticSurgeryQuizQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < cosmeticSurgeryQuizQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final ProgressService progressService = ProgressService();
    await progressService.saveProgress('cosmeticSurgeryCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final ProgressService progressService = ProgressService();
    await progressService.addToReviewList('cosmeticSurgeryReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: cosmeticSurgeryQuizQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(cosmeticSurgeryQuizQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${cosmeticSurgeryQuizQuestions.length}問',
