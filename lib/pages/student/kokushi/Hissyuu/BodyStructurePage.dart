...
class BodyStructurePage extends StatefulWidget {
  ...
  @override
  _BodyStructurePageState createState() => _BodyStructurePageState();
}

class _BodyStructurePageState extends State<BodyStructurePage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(humanBodyStructureQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(humanBodyStructureQuestions.length, 0);
      for (int i = 0; i < progress['bodyStructureCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('bodyStructureCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(humanBodyStructureQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < humanBodyStructureQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('bodyStructureCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('bodyStructureReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: humanBodyStructureQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(humanBodyStructureQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${humanBodyStructureQuestions.length}問',
