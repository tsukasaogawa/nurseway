...
class DailyLifeAssistancePage extends StatefulWidget {
  ...
  @override
  _DailyLifeAssistancePageState createState() => _DailyLifeAssistancePageState();
}

class _DailyLifeAssistancePageState extends State<DailyLifeAssistancePage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(dailyLifeAssistanceTechniquesQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(dailyLifeAssistanceTechniquesQuestions.length, 0);
      for (int i = 0; i < progress['dailyLifeAssistanceCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('dailyLifeAssistanceCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(dailyLifeAssistanceTechniquesQuestions.length, 0);
    });
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('dailyLifeAssistanceCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('dailyLifeAssistanceReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: dailyLifeAssistanceTechniquesQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(dailyLifeAssistanceTechniquesQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${dailyLifeAssistanceTechniquesQuestions.length}問',
