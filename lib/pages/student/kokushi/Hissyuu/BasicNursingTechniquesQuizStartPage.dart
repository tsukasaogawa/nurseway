...
import 'BasicNursingTechniquesPage.dart';
import 'hissyuu_model/hissyuu_quiz_data.dart';

class BasicNursingTechniquesQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const BasicNursingTechniquesQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    int totalQuestions = basicNursingTechniquesQuestions.length; // 問題の総数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('看護における基本技術', style: TextStyle(color: Colors.black)),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0
                    ? () async {
                        final result = await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => BasicNursingTechniquesPage(
