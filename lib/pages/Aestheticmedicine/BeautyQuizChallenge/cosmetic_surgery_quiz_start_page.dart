...
import 'cosmetic_surgery_quiz_page.dart';

class CosmeticSurgeryQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const CosmeticSurgeryQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('美容外科クイズ', style: TextStyle(color: Colors.black)),
                ...
                onPressed: lastCompletedQuestion > 0
                    ? () async {
                        final result = await Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => CosmeticSurgeryQuizPage(
