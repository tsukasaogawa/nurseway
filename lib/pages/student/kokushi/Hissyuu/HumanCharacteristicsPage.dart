...
class HumanCharacteristicsPage extends StatefulWidget {
  ...
  @override
  _HumanCharacteristicsPageState createState() => _HumanCharacteristicsPageState();
}

class _HumanCharacteristicsPageState extends State<HumanCharacteristicsPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(humanCharacteristicsQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(humanCharacteristicsQuestions.length, 0);
      for (int i = 0; i < progress['humanCharacteristicsCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('humanCharacteristicsCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(humanCharacteristicsQuestions.length, 0);
    });
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('humanCharacteristicsCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('humanCharacteristicsReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: humanCharacteristicsQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(humanCharacteristicsQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${humanCharacteristicsQuestions.length}問',
