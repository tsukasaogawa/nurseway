...
import 'other_medical_aesthetics_quiz_page.dart';

class OtherMedicalAestheticsQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const OtherMedicalAestheticsQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('その他の美容医療クイズ', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => OtherMedicalAestheticsQuizPage(
