...
class DrugManagementPage extends StatefulWidget {
  ...
  @override
  _DrugManagementPageState createState() => _DrugManagementPageState();
}

class _DrugManagementPageState extends State<DrugManagementPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(drugEffectsAndManagementQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(drugEffectsAndManagementQuestions.length, 0);
      for (int i = 0; i < progress['drugManagementCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('drugManagementCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(drugEffectsAndManagementQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < drugEffectsAndManagementQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('drugManagementCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('drugManagementReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: drugEffectsAndManagementQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(drugEffectsAndManagementQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${drugEffectsAndManagementQuestions.length}問',
