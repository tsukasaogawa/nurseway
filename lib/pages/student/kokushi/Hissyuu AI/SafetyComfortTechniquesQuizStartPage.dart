...
import 'SafetyComfortTechniquesPage.dart';
import 'hissyuu_model/hissyuu_quiz_data.dart';

class SafetyComfortTechniquesQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const SafetyComfortTechniquesQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    int totalQuestions = patientSafetyTechniquesQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('患者の安全・安楽を守る看護技術', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => SafetyComfortTechniquesPage(
