...
class HealthFactorsQuizStartPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    int totalQuestions = healthFactorsQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('健康に影響する要因', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => HealthFactorsPage(
