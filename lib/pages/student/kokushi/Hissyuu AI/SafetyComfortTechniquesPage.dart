...
class SafetyComfortTechniquesPage extends StatefulWidget {
  ...
  @override
  _SafetyComfortTechniquesPageState createState() => _SafetyComfortTechniquesPageState();
}

class _SafetyComfortTechniquesPageState extends State<SafetyComfortTechniquesPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(patientSafetyTechniquesQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(patientSafetyTechniquesQuestions.length, 0);
      for (int i = 0; i < progress['safetyComfortTechniquesCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('safetyComfortTechniquesCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(patientSafetyTechniquesQuestions.length, 0);
    });
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('safetyComfortTechniquesCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('safetyComfortTechniquesReviewList', questionIndex);
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('患者の安全・安楽を守る看護技術'),
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: patientSafetyTechniquesQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(patientSafetyTechniquesQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${patientSafetyTechniquesQuestions.length}問',
