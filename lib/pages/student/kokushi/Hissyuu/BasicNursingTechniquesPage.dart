...
class BasicNursingTechniquesPage extends StatefulWidget {
  ...
  @override
  _BasicNursingTechniquesPageState createState() => _BasicNursingTechniquesPageState();
}

class _BasicNursingTechniquesPageState extends State<BasicNursingTechniquesPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(basicNursingTechniquesQuestions.length, 0);
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('basicNursingTechniquesCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(basicNursingTechniquesQuestions.length, 0);
    });
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('basicNursingTechniquesReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: basicNursingTechniquesQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(basicNursingTechniquesQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${basicNursingTechniquesQuestions.length}問',
