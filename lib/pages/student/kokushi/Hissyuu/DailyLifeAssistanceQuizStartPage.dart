...
import 'DailyLifeAssistancePage.dart';
import 'hissyuu_model/hissyuu_quiz_data.dart';

class DailyLifeAssistanceQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const DailyLifeAssistanceQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    int totalQuestions = dailyLifeAssistanceTechniquesQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('日常生活援助技術', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => DailyLifeAssistancePage(
