...
class SymptomsDiseasesPage extends StatefulWidget {
  ...
  @override
  _SymptomsDiseasesPageState createState() => _SymptomsDiseasesPageState();
}

class _SymptomsDiseasesPageState extends State<SymptomsDiseasesPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(symptomsAndDiseasesQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(symptomsAndDiseasesQuestions.length, 0);
      for (int i = 0; i < progress['symptomsDiseasesCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('symptomsDiseasesCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(symptomsAndDiseasesQuestions.length, 0);
    });
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('symptomsDiseasesCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('symptomsDiseasesReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: symptomsAndDiseasesQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(symptomsAndDiseasesQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${symptomsAndDiseasesQuestions.length}問',
