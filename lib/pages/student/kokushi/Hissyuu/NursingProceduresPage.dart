...
class NursingProceduresPage extends StatefulWidget {
  ...
  @override
  _NursingProceduresPageState createState() => _NursingProceduresPageState();
}

class _NursingProceduresPageState extends State<NursingProceduresPage> with SingleTickerProviderStateMixin {
  late PageController _pageController;
  late AnimationController _animationController;
  late Animation<Offset> _positionAnimation;
  late Animation<double> _opacityAnimation;
  int _currentPage = 0;
  List<int> _completedQuestions = List.filled(nursingWithTreatmentTechniquesQuestions.length, 0);
  ...
  Future<void> _loadProgress() async {
    ...
    setState(() {
      _completedQuestions = List<int>.filled(nursingWithTreatmentTechniquesQuestions.length, 0);
      for (int i = 0; i < progress['nursingProceduresCompleted']!; i++) {
  ...
  void _resetProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('nursingProceduresCompleted', 0);
    setState(() {
      _completedQuestions = List<int>.filled(nursingWithTreatmentTechniquesQuestions.length, 0);
    });
  ...
  void _nextPage() {
    if (_currentPage < nursingWithTreatmentTechniquesQuestions.length - 1) {
  ...
  Future<void> _saveProgress() async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.saveProgress('nursingProceduresCompleted', _completedQuestions.where((q) => q == 1).length);
  ...
  Future<void> _addToReviewList(int questionIndex) async {
    final HissyuuProgressService progressService = HissyuuProgressService();
    await progressService.addToReviewList('nursingProceduresReviewList', questionIndex);
  }
        ...
        body: Stack(
          children: [
            PageView.builder(
              ...
              itemCount: nursingWithTreatmentTechniquesQuestions.length,
              onPageChanged: (page) {
                setState(() {
                  _currentPage = page;
                });
              ...
              itemBuilder: (context, index) {
                return SingleChildScrollView(
                  child: _buildQuestionPage(nursingWithTreatmentTechniquesQuestions[index], index),
                );
              },
            ),
  ...
  Widget _buildQuestionPage(QuizQuestion question, int index) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '第${index + 1}問 / ${nursingWithTreatmentTechniquesQuestions.length}問',
