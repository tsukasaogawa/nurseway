...
import 'HumanCharacteristicsPage.dart';
import 'hissyuu_model/hissyuu_quiz_data.dart';

class HumanCharacteristicsQuizStartPage extends StatelessWidget {
  final int lastCompletedQuestion;
  final void Function(int completedQuestions) onProgressUpdate;

  const HumanCharacteristicsQuizStartPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    int totalQuestions = humanCharacteristicsQuestions.length; // 実際の問題数を取得
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('人間の特性', style: TextStyle(color: Colors.black)),
              ...
              ElevatedButton(
                onPressed: lastCompletedQuestion > 0 ? () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => HumanCharacteristicsPage(
